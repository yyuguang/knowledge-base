...
sources:
  ...
  - "wiki/30-sources/repositories/来源_MyBatis_3_源码.md"
...
详见 [[10-domains/java/mybatis/概念_MyBatis核心生命周期与Configuration]] — 解释构建顺序、配置注册表和线程安全边界。
...
详见 [[10-domains/java/mybatis/概念_MyBatis_XML语句解析与MappedStatement构建]] — 解释一条 XML SQL 如何被编译为 `MappedStatement`。![[Attachments/mybatis-mappedstatement-build-flow.excalidraw]]
...
详见 [[10-domains/java/mybatis/概念_MyBatis执行器缓存与插件链]] — 覆盖 `SimpleExecutor/ReuseExecutor/BatchExecutor/CachingExecutor`、一级/二级缓存和插件链。
...
详见 [[10-domains/java/mybatis/概念_MyBatis_ResultSetHandler结果映射机制]] — 解释自动映射、嵌套映射、延迟加载、对象创建策略。![[Attachments/mybatis-resultsethandler-mapping-flow.excalidraw]]
...
详见 [[10-domains/java/mybatis/概念_MyBatis_Mapper代理与MappedStatement]] — 解释 statement id、返回值适配和 `MappedStatement` 构建结果。
...
相关已有页面：[[10-domains/java/spring/概念_Spring数据访问_JdbcTemplate_事务传播_MyBatis_多数据源]] — Spring 侧的整合机制入口。

## 核心概念

- [[10-domains/java/mybatis/概念_MyBatis核心生命周期与Configuration]] — 从资源解析到 `Configuration` 注册表。
- [[10-domains/java/mybatis/概念_MyBatis_XML语句解析与MappedStatement构建]] — 从 mapper XML statement 到 `MappedStatement`。
- [[10-domains/java/mybatis/概念_MyBatis_Mapper代理与MappedStatement]] — 从接口方法到 SQL 元数据。
- [[10-domains/java/mybatis/概念_MyBatis执行器缓存与插件链]] — 从执行器到 JDBC、缓存和插件。
- [[10-domains/java/mybatis/概念_MyBatis_ResultSetHandler结果映射机制]] — 从 JDBC `ResultSet` 到对象、集合、嵌套对象图和 lazy proxy。

## 关键来源

- [[30-sources/repositories/来源_MyBatis_3_源码]] — MyBatis 3 本地源码编译页。
...
- 已完成：第二轮已深挖 `XMLMapperBuilder -> XMLStatementBuilder -> MapperBuilderAssistant -> MappedStatement.Builder`，见 [[10-domains/java/mybatis/概念_MyBatis_XML语句解析与MappedStatement构建]]。
- 已完成：第三轮已深挖 `DefaultResultSetHandler` 的自动映射、嵌套映射、延迟加载和对象创建策略，见 [[10-domains/java/mybatis/概念_MyBatis_ResultSetHandler结果映射机制]]。
