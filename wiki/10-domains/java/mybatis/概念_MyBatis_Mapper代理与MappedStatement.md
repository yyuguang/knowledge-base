...
- [[10-domains/java/mybatis/概念_MyBatis核心生命周期与Configuration]] — `MapperRegistry` 和 `MappedStatement` 都存放在 `Configuration` 中。
- [[10-domains/java/mybatis/概念_MyBatis_XML语句解析与MappedStatement构建]] — XML mapper 在启动期构建出 Mapper 代理运行期要查找的 `MappedStatement`。
- [[10-domains/java/mybatis/概念_MyBatis执行器缓存与插件链]] — Mapper 方法转换为 `SqlSession` 调用后，进入执行器和缓存链。
- [[10-domains/java/mybatis/主题_MyBatis源码架构_综述]] — 将 Mapper 代理放回 MyBatis 整体架构中理解。
