...
从设计模式看，这条主线其实是多种模式的组合：ChatClient 是门面，Builder 负责复杂配置，Advisor 是责任链和模板方法，RAG 是策略流水线，provider 是适配器，AutoConfiguration 是 Spring 风格工厂。详见 [[10-domains/java/spring-ai/概念_Spring_AI_设计模式]] — 用模式解释这些源码结构为什么这样组织。
...
- [[10-domains/java/spring-ai/概念_Spring_AI_ChatClient_API]] — ChatClient 请求构建与执行入口。
- [[10-domains/java/spring-ai/概念_Spring_AI_Advisor拦截链]] — Advisor 链的排序、消费式执行和复制机制。
- [[10-domains/java/spring-ai/概念_Spring_AI_ToolCalling工具调用]] — ToolCallAdvisor 递归循环和 ToolCallingManager。
- [[10-domains/java/spring-ai/概念_Spring_AI_RAG检索增强生成]] — RetrievalAugmentationAdvisor 的 Modular RAG 实现。
- [[10-domains/java/spring-ai/概念_Spring_AI_AutoConfiguration自动配置]] — Spring Boot 条件装配与 starter 依赖结构。
- [[10-domains/java/spring-ai/概念_Spring_AI_设计模式]] — 从设计模式角度解释 Spring AI 为什么这样组织源码。
