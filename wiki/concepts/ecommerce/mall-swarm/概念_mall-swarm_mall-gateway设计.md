...
- [[concepts/ecommerce/mall-swarm/概念_mall-swarm_认证网关与部署安全]]：第二轮 P0 对 Docker/Kubernetes 旁路、白名单、CORS、下游防伪造与 AI 委托的交叉核查。
- [[overview/ecommerce/mall-swarm/主题_mall-swarm_架构全景_综述]]：全服务运行时边界与后续学习路线。
- [[sources/java/mall-swarm/来源_mall-swarm_项目入口与模块地图]]：模块、Nacos、路由的第一轮证据地图。
- [[concepts/ecommerce/mall-swarm/概念_mall-swarm_mall-common设计]]：`CommonResult`、`AuthConstant`、Redis 与 MVC/WebFlux 共享边界。
- [[concepts/java/spring-framework/概念_Spring_WebFlux响应式处理链]]：理解 Gateway 所在 WebFlux 请求链的框架基础。
