...
- [[concepts/java/mybatis/概念_MyBatis核心生命周期与Configuration]] — 执行器和 handler 都由 `Configuration` 创建。
- [[concepts/java/mybatis/概念_MyBatis_Mapper代理与MappedStatement]] — Mapper 调用最终定位 `MappedStatement` 后进入执行器链。
- [[concepts/java/mybatis/概念_MyBatis_ResultSetHandler结果映射机制]] — `StatementHandler.query()` 后，`ResultSetHandler` 负责把 JDBC 行集映射为对象图。
- [[concepts/java/spring-framework/概念_Spring_Framework_数据访问事务同步与集成模块]] — Spring 事务整合会影响 MyBatis 事务和连接获取方式。
