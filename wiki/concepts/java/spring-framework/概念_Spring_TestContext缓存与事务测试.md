...
相关主线：[[concepts/java/spring-framework/概念_Spring_Framework_Web测试消息与应用层扩展]] — TestContext 属于应用层策略模型；[[concepts/java/spring-framework/概念_Spring_Framework_IoC容器启动与Bean生命周期]] — 测试上下文缓存复用的是完整 ApplicationContext。
...
- [[concepts/java/spring-framework/概念_Spring_WebFlux响应式处理链]] — WebTestClient 是验证 WebFlux 响应式链路的主要工具。
- [[concepts/java/spring-framework/概念_Spring_Framework_IoC容器启动与Bean生命周期]] — ContextCache 缓存的是已经 refresh 完成的 ApplicationContext。
- [[concepts/java/spring-framework/概念_Spring_AOP责任链源码时序]] — 测试事务和声明式业务事务都依赖 Spring transaction 抽象，但入口不同。
