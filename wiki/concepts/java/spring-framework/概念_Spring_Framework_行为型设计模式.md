...
- [[concepts/java/spring-framework/概念_Spring_Framework_IoC容器启动与Bean生命周期]] — `refresh()` 是模板方法的核心例子。
- [[concepts/java/spring-framework/概念_Spring_Framework_数据访问事务同步与集成模块]] — JdbcTemplate、TransactionTemplate 展示模板+回调。
- [[concepts/java/spring-framework/概念_Spring_Framework_Web测试消息与应用层扩展]] — Web 参数解析、返回值处理、拦截器链体现策略和责任链。
- [[overview/java/spring-framework/主题_Spring_Framework设计模式_综述]] — 设计模式总览。
