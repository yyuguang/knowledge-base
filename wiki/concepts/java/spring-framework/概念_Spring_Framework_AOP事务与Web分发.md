...
- [[concepts/java/spring-framework/概念_Spring_Framework_IoC容器启动与Bean生命周期]] — AOP 代理通常由 BeanPostProcessor 在 Bean 初始化阶段产生。
- [[concepts/java/spring-framework/概念_Spring_Framework_模块体系与扩展点]] — AOP、事务、Web 都是模块通过扩展点接入内核的例子。
- [[sources/java/spring-framework/来源_Spring_Framework_源码]] — 本页关键源码入口来自 source page。
