...
相关主线：[[concepts/java/spring-framework/概念_Spring_Framework_行为型设计模式]] — AOP 是责任链和回调模式的典型源码实例；[[concepts/java/spring/概念_Spring_AOP模块_动态代理_通知_切点_事务代理]] — AOP 模块面试主线。
...
- [[concepts/java/spring-framework/概念_Spring_Framework_结构型设计模式]] — proxy 结构让客户端无感调用增强对象。
- [[concepts/java/spring-framework/概念_Spring_Framework_行为型设计模式]] — advisor chain 是责任链，`invocation.proceed()` 是回调。
- [[concepts/java/spring-framework/概念_Spring_TestContext缓存与事务测试]] — 测试事务入口是 TestExecutionListener，业务声明式事务入口是 AOP chain。
