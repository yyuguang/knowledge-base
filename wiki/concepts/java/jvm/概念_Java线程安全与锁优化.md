...
- [[concepts/java/jvm/概念_Java内存模型JMM]] 提供理论基础，本页是实践手段

## 关联

- [[concepts/java/jvm/概念_Java内存模型JMM]] — JMM 定义原子性/可见性/有序性规则，本页的 synchronized 和 CAS 是实现这些规则的具体手段
- [[concepts/java/jvm/概念_JIT编译优化技术]] — 锁消除依赖逃逸分析判断对象是否线程私有，偏向锁的批量撤销涉及 JIT 编译
- [[entities/java/jvm/项目_HotSpot_VM]] — 锁膨胀路径（偏向锁→轻量级锁→重量级锁）是 HotSpot 独有的实现细节
