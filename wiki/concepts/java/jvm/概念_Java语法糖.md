...
- [[concepts/java/jvm/概念_Class文件结构]] 的 Signature 属性保留泛型元数据

## 关联

- [[concepts/java/jvm/概念_Class文件结构]] — 语法糖的编译结果体现在 Class 文件中，如 Signature 属性保留泛型元数据，Code 属性记录还原后的字节码
- [[concepts/java/jvm/概念_JIT编译优化技术]] — 前端编译处理语法糖后，JIT 在后端对结果做深层优化
- [[concepts/java/jvm/概念_字节码执行引擎]] — Lambda 表达式编译为 invokedynamic 指令后，由执行引擎在运行时解析调用目标
