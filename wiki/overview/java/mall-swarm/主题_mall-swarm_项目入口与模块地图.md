...
sources:
  - "wiki/sources/java/mall-swarm/来源_mall-swarm_项目入口与模块地图.md"
...
图中路由、端口、依赖箭头均来自 [[sources/java/mall-swarm/来源_mall-swarm_项目入口与模块地图]] 所列 POM 与配置。图中不表达具体业务调用或数据库表归属。
...
这不是当前代码已实现的 AI 架构，而是基于现有边界做出的演进建议；落地前可对照 [[overview/java/spring-ai/主题_Spring_AI源码架构_综述]] 的 ChatClient、Advisor、Tool Calling 与观测扩展点。

## 与既有知识的连接

- [[concepts/java/spring/概念_Spring_Boot内核_自动配置_条件注解_启动过程_Starter]]：解释服务启动时 starter 与自动配置如何接入。
- [[concepts/java/spring/概念_Spring_Web层_DispatcherServlet_HandlerMapping_参数返回值_过滤器拦截器]]：下游 MVC 服务的 controller 调用链可用此页做阅读参照；网关本身为 WebFlux，不能直接等同。
- [[concepts/java/mybatis/概念_MyBatis核心生命周期与Configuration]]：后续从 `mall-mbg` 深入数据库访问时的运行时基础。
- [[overview/java/spring-ai/主题_Spring_AI源码架构_综述]]：后续评估 AI 适配层的候选模式。
